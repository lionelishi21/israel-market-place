            ...
            <div class="container">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-9 main-content">
                        ...
                        <div class="row">
                            <div class="col-6 col-sm-4" v-for="prod in products" :key="prod.id">
                                <product-one :product="prod"></product-one>
                            </div>
                        </div>
                        <!-- End .row -->
                        ...
                        <div class="sidebar-wrapper">
                            <div class="widget">
                                <h3 class="widget-title">
                                    <a data-toggle="collapse" href="#widget-body-2" role="button" aria-expanded="true" aria-controls="widget-body-2">Categorie</a>
                                </h3>
                                    ...
                                    <div class="widget-body">
                                        <ul class="cat-list">
                                            <li v-for="category in categories" :key="category.id" >
                                                <div >
                                                    <a @click.prevent="selectCategory(category)" v-if="category.parent_id === false" href="#" data-toggle="collapse" role="button" aria-expanded="true" aria-controls="widget-category-1">
                                                        {{category.name}}<span class="products-count">({{ category.product_tmpl_ids.length }})</span>
                                                        <span class="toggle"></span>
                                                    </a>

                                                    <div class="collapse show" id="widget-category-1">
                                                        <div>
                                                            <ul class="cat-sublist" v-for="sub in subCategories(category.child_id)" :key="sub.id">
                                                                <li>{{sub.name}}
                                                                    <span class="products-count">({{sub.product_count}})</span>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
...
import ProductOne from '~/components/theme4/features/product/ProductOne.vue'

const loading = ref(false)
const filter = ref({})
const productCategory = useProductCategoryStore();
const product = await useProductStore();
...
const filterProducts =  async  () => {
    ...
    if (route.query.category) {
        const params = route.query.category.split("-")
...
const subCategories = (params: any) => {

    var response = [];
    for(let i = 0; i < params.length; i++) {
        let category = productCategory.listProductCategories.find((item: number) => item.id === params[i]);
        let arr = {id: params[i], name: category.name, product_count: category.product_tmpl_ids.length, products: category.category_tmpl_ids }
        response.push(arr)
    }
    return response;
}
...
const products = computed(() => product.listFilterProducts)
</script>
<style>
.product-image {
